...
def constructBayesNet(gameState: hunters.GameState):
    ...
    #add the variables and edges to the list
    variables = [GHOST1, OBS1, OBS0, GHOST0, PAC]
    edges = [(GHOST1, OBS1), (PAC, OBS1), (GHOST0, OBS0), (PAC, OBS0)]

    "*** YOUR CODE HERE ***"

    #create a dictionary with the nodes as keys and empty lists as values
    variableDomainsDict = dict.fromkeys([PAC, GHOST0, GHOST1, OBS0, OBS1])
    for key, _ in variableDomainsDict.items():
        variableDomainsDict[key] = []

    #add values to the domains 
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            #add all possible grid positions
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        #get a list of all CPTs
        factList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        
        #eliminate from the order given
        for var in eliminationOrder:
            factList, joinedFactor = joinFactorsByVariable(factList, var)
            
            size = len(joinedFactor.unconditionedVariables())
            #eliminate only if there are more than one factors
            if  (size > 1):
                #remove that variable
                elem = eliminate(joinedFactor, var)
                #append it to the list of factors
                factList.append(elem)
        
        #return the normalised factors
        return normalize(joinFactors(factList))
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
